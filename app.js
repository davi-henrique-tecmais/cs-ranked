...
function generateExpMatchFromSetup() {
  ...
  if (selectedPlayerIds.length < 2) {
    window.alert("Select at least 2 players to generate the EXP match.");
    return;
  }

  const shuffledPlayers = shuffleIds(selectedPlayerIds);
  const splitIndex = Math.ceil(shuffledPlayers.length / 2);
  const teamAIds = shuffledPlayers.slice(0, splitIndex);
  const teamBIds = shuffledPlayers.slice(splitIndex);
  const match = {
    id: makeId(),
    createdAt: new Date().toISOString(),
...
function renderExpMatchDialog(match) {
  ...
  renderExpTeamList(elements.expTeamAList, match.teamAIds, goatId);
  renderExpTeamList(elements.expTeamBList, match.teamBIds, goatId);
  renderExpStatsForm(match);
  updateExpResultPreview();
}

function renderExpTeamList(list, playerIds, goatId = null) {
  list.innerHTML = "";

  playerIds.forEach((playerId) => {
    const player = getExpPlayer(playerId);
    const item = document.createElement("li");
    item.classList.toggle("goat-player", playerId === goatId);
    item.innerHTML = `
      <span>${escapeHtml(player?.name ?? "Removed player")}${playerId === goatId ? getGoatStarMarkup() : ""}</span>
      <span class="bot-pill">EXP</span>
    `;
...
function renderExpStatsForm(match) {
  ...
  const goatId = getExpGoatId(match);
  elements.expStatsGrid.innerHTML = `
    <div class="exp-stats-header">Player</div>
    <div class="exp-stats-header">Kills</div>
    <div class="exp-stats-header">Deaths</div>
    <div class="exp-stats-header">Assists</div>
    <div class="exp-stats-header">MVP</div>
    <div class="exp-stats-header">Score</div>
  `;

  playerIds.forEach((playerId) => {
    const player = getExpPlayer(playerId);
    const teamLabel = match.teamAIds.includes(playerId) ? "Team A" : "Team B";
    const stats = match.playerStats[playerId] ?? normalizeExpPlayerStats(null);
    const row = document.createElement("div");
    row.className = "exp-stats-row";
    row.innerHTML = `
    ...
    `;
    elements.expStatsGrid.appendChild(row);
  });
}
...
function deleteSelectedExpMatch() {
  ...
  const confirmed = window.confirm(`Delete the EXP match on ${match.map.toUpperCase()}?`);
  if (!confirmed) return;
